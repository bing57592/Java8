...
/**
  ...
  *
  * Created by 陈宪东 on 2018/12/27 2:38  
  */
object Share5_Demo_CDN extends App {
  // 获取spark操作对象, 获取rdd算子.
  var sparkConf = new SparkConf().setMaster("local[*]").setAppName("clickcount")
  var sc = new SparkContext(sparkConf)
  var rdd_requests = sc.textFile("C:\\Users\\chenxiandong\\Desktop\\input\\cdn.txt")

  val count = rdd_requests.map(item => (1, 1)).reduceByKey(_ + _)
  ...
  val rdd2 = rdd1.map(item => { // 元数据 => (视频名称_IP)  (a.mp4_ip1)(a.mp4_ip1)(a.mp4_ip1)
    val params = item.split(" ")
    val url = params(6)
    val ip = params(0)
  ...
  val rdd3 = rdd2.map(item => { // (去重) (视频名称_IP => (视频名称, IP))
    ...
    println(params)
    (params(0), params(1))
  }).groupByKey()//(a.mp4, ip1) (a.mp4, ip2) => (a.mp4, List(ip1, ip2))

  val rdd4 = rdd3.map(item => {
    // [对key聚集] (视频名称, IP) => 视频名称, List(IP)
    println(item)
